...
static constexpr uint16_t gStrokeCircleIndices[] = {
    0, 1, 9,  0, 9,  8,  1, 2, 10, 1, 10, 9,  2, 3, 11, 2, 11, 10, 3, 4, 12, 3, 12, 11,
    4, 5, 13, 4, 13, 12, 5, 6, 14, 5, 14, 13, 6, 7, 15, 6, 15, 14, 7, 0, 8,  7, 8,  15,
};
...
ShadowCircularRRectOp::ShadowCircularRRectOp(QRgb color, const QRectF& devRect, QVector4D devRadius,
                                             bool isCircle, float blurRadius, float insetWidth)
    : fIndexPtr(nullptr) {
    ...
    if (isCircle) {
        ...
        if (insetWidth <= 0.5f * std::min(devRect.width(), devRect.height())) {
            float maxRadius =
                std::max({ devRadius.x(), devRadius.y(), devRadius.z(), devRadius.w() });
    ...
    if (isCircle) {
        ...
        fIndexCount = (kStroke_RRectType == type) ? std::size(gStrokeCircleIndices)
                                                  : std::size(gFillCircleIndices);
        fIndexPtr   = circle_type_to_indices(kStroke_RRectType == type);
    } else {
        switch (type) {
        case kFill_RRectType:
            fVertCount  = kVertsPerFillRRect;
            fIndexCount = std::size(gRRectIndices) - 6 * 4;
            break;
        case kStroke_RRectType:
            fVertCount  = kVertsPerStrokeRRect;
            fIndexCount = std::size(gRRectIndices) - 6 * 4 - 6;
            break;
        case kOverstroke_RRectType:
            fVertCount  = kVertsPerOverstrokeRRect;
            fIndexCount = std::size(gRRectIndices) - 6;
            break;
        }
...
void ShadowCircularRRectOp::fillInCircleVerts(bool isStroked, sg::ShadowVertex** vp) const {
    ...
    auto      v          = vp[0];
    QVector2D offsets[8] = { { -octOffset, -1.0f }, { octOffset, -1.0f }, { 1.0f, -octOffset },
                             { 1.0f, octOffset },   { octOffset, 1.0f },  { -octOffset, 1.0f },
                             { -1.0f, octOffset },  { -1.0f, -octOffset } };
    ...
    if (isStroked) {
        ...
        QVector2D iOffsets[8] = { { -s, -c }, { s, -c }, { c, -s }, { c, s },
                                  { s, c },   { -s, c }, { -c, s }, { -c, -s } };
        for (int i = 0; i < 8; ++i) {
...
void ShadowCircularRRectOp::fillInRRectVerts(sg::ShadowVertex** vp) const {
    ...
    float  maxR                = std::max({ args.outer_radius.x(),
                                            args.outer_radius.y(),
                                            args.outer_radius.z(),
                                            args.outer_radius.w() });
    ...
    auto v = vp[0];
    sg::mesh::for_each_rrect_corner(
        bounds, args.outer_radius, umbra_inset, [&](const sg::mesh::rrect_corner& c) {
            ...
            scalar    diagVal =
                (std::abs(denom) > k_nearly_zero) ? umbra_inset / denom : -1.0f / k_float_sqrt2;
