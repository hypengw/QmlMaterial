...
void update_rectangle_geometry(RectangleVertex* v, QVector2D size, QRgb color, QVector4D radius) {
    ...
    mesh::build_nine_patch(
        v, size, radius, [&](RectangleVertex* cell, int corner, QVector2D origin, QVector2D csize) {
...
void update_shadow_geometry(QSGGeometry* geo, const ShadowParams& params, const QRectF& rect) {
    ...
    bool is_oval = qFuzzyCompare((float)params.radius.x(), max_radius) &&
                   qFuzzyCompare((float)params.radius.y(), max_radius) &&
                   qFuzzyCompare((float)params.radius.z(), max_radius) &&
                   qFuzzyCompare((float)params.radius.w(), max_radius) &&
    ...
    if (params.ambient_color > 0) {
        ...
        math::scalar devSpaceAmbientBlur =
            devSpaceInsetWidth * math::ambient_recip_alpha(occluderHeight);
        QRectF ambientRRect = rect.adjusted(
            -devSpaceInsetWidth, -devSpaceInsetWidth, devSpaceInsetWidth, devSpaceInsetWidth);
        QVector4D ambientRadius = params.radius + QVector4D(devSpaceInsetWidth,
                                                            devSpaceInsetWidth,
                                                            devSpaceInsetWidth,
                                                            devSpaceInsetWidth);
        for (int i = 0; i < 4; ++i)
            ambientRadius[i] = std::min<float>(ambientRadius[i], max_radius + devSpaceInsetWidth);
    ...
    if (params.spot_color > 0) {
        math::scalar blur, scale;
        QVector2D    offset;
        if (directional)
            math::directional_params(occluderHeight,
                                     params.light_pos.x(),
                                     params.light_pos.y(),
                                     params.light_pos.z(),
                                     params.light_radius,
            ...
            math::spot_params(occluderHeight,
                              params.light_pos.x(),
                              params.light_pos.y(),
                              params.light_pos.z(),
                              params.light_radius,
        ...
        if (transparent)
            insetWidth += spotRRect.width();
        else {
            float maxR = std::max(
                { params.radius.x(), params.radius.y(), params.radius.z(), params.radius.w() });
            math::scalar dr = maxR * scale - maxR;
            QVector2D    offUL =
                QVector2D(spotRRect.left() - rect.left() + dr, spotRRect.top() - rect.top() + dr);
            QVector2D offBR = QVector2D(spotRRect.right() - rect.right() - dr,
                                        spotRRect.bottom() - rect.bottom() - dr);
            insetWidth += std::max(
                blur,
                (math::scalar)std::sqrt(std::max(offUL.lengthSquared(), offBR.lengthSquared())) +
                    dr);
        }
        ...
        ops[1] = sk::ShadowCircularRRectOp(params.spot_color,
                                           spotRRect,
                                           spotRadius + QVector4D(blur, blur, blur, blur),
                                           is_oval,
                                           2.0f * blur,
                                           insetWidth);
    }
    ...
    for (auto& p : ops)
        if (p) {
            v_count += p->fVertCount;
            i_count += p->fIndexCount;
        }
    ...
    for (auto& p : ops)
        if (p) {
            if (p->fGeoData.is_circle)
                p->fillInCircleVerts(
                    sk::ShadowCircularRRectOp::kStroke_RRectType == p->fGeoData.type, &v);
            else
                p->fillInRRectVerts(&v);
