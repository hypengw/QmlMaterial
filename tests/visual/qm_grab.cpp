...
int main(int argc, char* argv[]) {
    ...
    QRhiTexture* colorTex = rhi->newTexture(
        QRhiTexture::RGBA8, sz, 1, QRhiTexture::RenderTarget | QRhiTexture::UsedAsTransferSource);
    if (! colorTex->create()) {
        std::cerr << "Failed to create color texture\n";
        return 1;
    }

    QRhiRenderBuffer* dsRb = rhi->newRenderBuffer(
        QRhiRenderBuffer::DepthStencil, sz, 1, QRhiRenderBuffer::UsedWithSwapChainOnly);
    ...
    if (! envPath.isEmpty()) {
        for (const auto& p : QString::fromLocal8Bit(envPath).split(':', Qt::SkipEmptyParts))
            engine.addImportPath(p);
    }

    QQmlComponent component(&engine, QUrl::fromLocalFile(QFileInfo(inputQml).absoluteFilePath()));
