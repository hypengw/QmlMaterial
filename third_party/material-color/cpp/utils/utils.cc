...
double DiffDegrees(const double a, const double b) { return 180.0 - abs(abs(a - b) - 180.0); }
...
std::string HexFromArgb(Argb argb) {
    ...
    std::string                out;
    for (int i = 0; i < 4; i++) {
        auto b = std::byte { (unsigned char)((argb >> i) & 0xff) };

        auto idx = std::to_integer<std::size_t>((b << 4) >> 4);
        out.push_back(hex_digits[idx]);
        idx = std::to_integer<std::size_t>(b >> 4);
        out.push_back(hex_digits[idx]);
    }
...
Vec3 MatrixMultiply(Vec3 input, const double matrix[3][3]) {
    double a = input.a * matrix[0][0] + input.b * matrix[0][1] + input.c * matrix[0][2];
    double b = input.a * matrix[1][0] + input.b * matrix[1][1] + input.c * matrix[1][2];
    double c = input.a * matrix[2][0] + input.b * matrix[2][1] + input.c * matrix[2][2];
    return Vec3{a, b, c};
}
} // namespace material_color_utilities
